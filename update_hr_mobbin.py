...
hr_dir = '/home/ruanf/ZEPH/Z.CORE/src/pages/hr'
files = glob.glob(os.path.join(hr_dir, '*.jsx'))
...
for file_path in files:
    ...
    if "EmployeeManagement.jsx" in file_path:
        content = content.replace(
            "title=\"Gestão de Colaboradores\"",
            "title={<>Gestão de Colaboradores <span style={{ color: 'var(--text-primary)', fontWeight: 'var(--font-black)', opacity: 0.9 }}>Executive</span></>}"
        )
    if "TimeTracking.jsx" in file_path:
        content = content.replace(
            "title=\"Controle de Ponto\"",
            "title={<>Controle de Ponto <span style={{ color: 'var(--text-primary)', fontWeight: 'var(--font-black)', opacity: 0.9 }}>Executive</span></>}"
        )
...
print("HR mobbin script complete.")
