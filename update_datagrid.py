...
old_file = 'src/components/ui/DataGrid.jsx'

with open(old_file, 'r') as f:
    content = f.read()

new_content = """import React, { useRef, useState, useMemo, useCallback, memo } from 'react';
...
const DataGrid = ({
    ...
    const gridTemplateColumns = useMemo(() => {
        ...
        if (selectable) cols.unshift('48px'); // Checkbox column
        return cols.join(' ');
    ...
    const toggleRowSelection = (index, row) => {
        const newSet = new Set(selectedRows);
        if (newSet.has(index)) newSet.delete(index);
        else newSet.add(index);
        setSelectedRows(newSet);
        if (onSelectionChange) onSelectionChange(Array.from(newSet).map(idx => sortedData[idx]));
    };
    ...
    const toggleAllSelection = () => {
        if (selectedRows.size === sortedData.length) {
            ...
            const newSet = new Set(sortedData.map((_, i) => i));
            setSelectedRows(newSet);
    ...
    return (
        <div className={`ui-data-grid-wrapper ${className}`} style={{
            ...
            {selectable && selectedRows.size > 0 && (
                <div style={{
                    padding: '8px 16px', background: 'var(--bg-selected)', borderBottom: '1px solid var(--border-color)',
                    display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 'var(--text-sm)', color: 'var(--color-accent)', fontWeight: '600'
                }}>
            ...
            <div className="ui-data-grid-header" style={{
                ...
                borderBottom: '1px solid var(--border-color)', width: '100%', paddingRight: '12px'
            }}>
                {selectable && (
                    <div style={{
                    ...
                    return (
                        <div
                            ...
                            style={{
                                ...
                                justifyContent: getAlignment(col.align), fontWeight: '600', fontSize: 'var(--text-sm)',
                        ...
                        >
                            ...
                            {sortData && (
                                <span style={{ marginLeft: '4px', color: 'var(--color-info)' }}>
            ...
            <div ref={parentRef} className="scroll-container hide-scrollbar" style={{ flex: 1, overflow: 'auto', position: 'relative' }}>
                {isLoading ? (
                    ...
                    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', padding: '40px' }}>
                        ...
                        {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                            ...
                            return (
                                <MemoizedRow
                                    ...
                                    onSelect={() => toggleRowSelection(virtualRow.index, row)}
                                />
                            );
                        })}
                    </div>
